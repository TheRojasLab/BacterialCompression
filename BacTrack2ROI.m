%BacTrack2.m
%Tracks bacterial growth from phase image stacks for a specified region of interest...
...
%analysisregion:window of CellASIC chip to center on e.g. 1400uM
%
%OUTPUT:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%User Input
basename='14_6_21';%Name of the image stack, used to save file.
dirname=['C:\Users\GuyMa\Documents\Rojas Lab\Exported Movies\' basename '/' basename '_1_a'];%Directory that the image stack is saved in
...
thresh2=0.004;%Threshold used in edge finder. Default:0.01 %lower = more edges but more time
wthresh=1;%Threshold used to find cell septa, default=1.
analysisregion=1400;
analysisdist=analysisregion; %Where to center the analysis region (in uM)
livetrack=0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if recrunch==1
    ...
    analysisdistance=analysisdist/100; %2
...
%adding filtering step here to hopefully better filter posts
[rows cols]=size(lcell);

for i = 1:rows
for t = 1:cols
    
    verify = isnan(lcell(i,t));
    if verify ==1
    pixels{i,t}= []; 
    B{i,t}= [];
  %   lcell(i,t)=NaN;
...
save([basename '_' analysisdist '_BT' ])
save([basename '_' analysisdist '_BTlab' ],'labels','labels2','-v7.3')
